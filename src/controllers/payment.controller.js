...
async function markDepositAndMaybeConfirm(booking, amount) {
  ...
  if (
    (booking.paidAmount || 0) >= (booking.totalPrice || Number.MAX_SAFE_INTEGER)
  ) {
...
export const vnpReturn = async (req, res) => {
  try {
    ...
    if (!ok) {
      ...
      }/payment?status=failed&reason=invalid_sig`;
    ...
    if (!booking) {
      const redirect = `${
        process.env.FRONTEND_URL || "http://localhost:5173"
      }/payment?status=failed&reason=notfound`;
      return res.redirect(redirect);
    }
    ...
    if (rsp === "00") {
      ...
      const ref =
        q.vnp_TransactionNo || q.vnp_TransactionStatus || Date.now().toString();
      if (
        !booking.paymentRefs?.some(
          (p) => p.provider === "vnpay" && p.ref === ref
        )
      ) {
      ...
      const redirect = `${
        process.env.FRONTEND_URL || "http://localhost:5173"
      }/payment?status=success&code=${booking.code}`;
      return res.redirect(redirect);
    } else {
      const redirect = `${
        process.env.FRONTEND_URL || "http://localhost:5173"
      }/payment?status=failed&reason=${rsp}`;
      return res.redirect(redirect);
    }
  } catch (err) {
    const redirect = `${
      process.env.FRONTEND_URL || "http://localhost:5173"
    }/payment?status=failed&reason=server`;
    return res.redirect(redirect);
  }
};
...
export const vnpIpn = async (req, res) => {
  try {
    ...
    const ref =
      q.vnp_TransactionNo || q.vnp_TransactionStatus || Date.now().toString();
    if (
      booking.paymentRefs?.some((p) => p.provider === "vnpay" && p.ref === ref)
    ) {
